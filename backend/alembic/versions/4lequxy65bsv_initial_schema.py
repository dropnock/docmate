...
def upgrade() -> None:
    # --- Enums ---
    orgtype_enum = sa.Enum("digitizing_entity", "customer", name="orgtype")
    userrole_enum = sa.Enum(
        "admin", "de_supervisor", "de_indexer", "de_qa_agent",
        "customer_supervisor", "customer_qc_agent", name="userrole"
    )
    portal_enum = sa.Enum("digitizing", "customer", name="portal")
    s3status_enum = sa.Enum("provisioning", "ready", "error", name="s3bucketstatus")
    ...
    tasktype_enum = sa.Enum("indexing", "qa", "qc", name="tasktype")
    taskstatus_enum = sa.Enum(
        "pending", "in_progress", "completed", "failed", "stale", name="taskstatus"
    )
    aqlstatus_enum = sa.Enum("normal", "tightened", "reduced", name="aqlstatus")
    auditentity_enum = sa.Enum(
        "record", "task", "batch", "project", "user", name="auditentitytype"
    )
    ...
    for e in [
        orgtype_enum, userrole_enum, portal_enum, s3status_enum, batchstatus_enum,
        recordstatus_enum, versionreason_enum, tasktype_enum, taskstatus_enum,
        aqlstatus_enum, auditentity_enum, auditaction_enum,
    ]:
